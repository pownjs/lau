...
const listAlienvaultURIs = async function*(domain, options) {
    ...
    for (;;) {
        ...
        const data = await requestJSON(scheduler, { uri: `https://otx.alienvault.com/otxapi/indicator/hostname/url_list/${encodeURIComponent(domain)}?limit=${pageSize}&page=${page}`, headers, retry, timeout })
