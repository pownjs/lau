...
const listWebArchiveURIs = async function*(domain, options) {
    ...
    while (true) {
        ...
        const data = await requestJSON(scheduler, { uri: `http://web.archive.org/cdx/search/cdx?url=${encodeURIComponent(`${wildcard}${domain}`)}/*&output=json&collapse=urlkey&fl=original&limit=${pageSize}&offset=${offset}`, headers, retry, timeout })
